...
public class VideoUtil {
    ...
    //检查视频时间是否一致
    public static Boolean checkVideoTime(String ffmpegPath,String source, String target) {
        String source_time = getVideoTime(ffmpegPath,source);
        //取出时分秒
        assert source_time != null;
        source_time = source_time.substring(0, source_time.lastIndexOf("."));
        String target_time = getVideoTime(ffmpegPath,target);
        //取出时分秒
        assert target_time != null;
        target_time = target_time.substring(0, target_time.lastIndexOf("."));
        return source_time.equals(target_time);
    }

    //获取视频时间(时：分：秒：毫秒)
    public static String getVideoTime(String ffmpegPath,String video_path) {
        /*
        ffmpeg -i  lucene.mp4
         */
        ...
        try {
            ...
            String outstring = waitFor(ffmpegPath,p);
    ...
    public static String waitFor(String ffmpegPath,Process p) {
        InputStream in = null;
